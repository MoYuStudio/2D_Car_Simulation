...
func control():
	...
	if Input.is_action_pressed("accelerate"):
		current_acc = lerpf(current_acc, max_acc, 0.05) # switching gears
		...
		current_acc = lerpf(current_acc, 0, 0.001) # if ive released accelerate, engine is slowing down but doesnt stop immediately
	acc = target_acc
